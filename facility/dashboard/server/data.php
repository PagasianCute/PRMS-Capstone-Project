<?php
$sql2 = "SELECT m.* FROM messages m
         WHERE (m.receiver_id = {$row['fclt_id']} OR m.sender_id = {$row['fclt_id']})
         AND (m.receiver_id = {$fclt_id} OR m.sender_id = {$fclt_id})
         AND m.id = (SELECT MAX(id) FROM messages
                     WHERE (receiver_id = {$row['fclt_id']} OR sender_id = {$row['fclt_id']})
...
if ($row2 !== null) {
    ...
    $output .= '<div class="referral-card message-contact" data-contact-id="' . $row['fclt_id'] . '" data-message-id="' . $row2['id'] . '">
    ...
    // Handle the case where $row2 is null (no rows returned)
