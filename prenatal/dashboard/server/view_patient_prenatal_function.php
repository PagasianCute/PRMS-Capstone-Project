...
if (isset($_GET['trimester_table'])) {
    ...
    $currentCount = $records_count !== '' ? $records_count : $countData['record_count'];
    ...
    $firstQuery = "
        ...
        WHERE tt.patients_id = ? AND tt.check_up = ? AND tt.records_count = ?
        ORDER BY tt.records_count DESC
        LIMIT 1";

    $firstdataStmt = mysqli_prepare($conn, $firstQuery);

    mysqli_stmt_bind_param($firstdataStmt, "isi", $patient_id, $checkup, $currentCount);
    mysqli_stmt_execute($firstdataStmt);

    $firstdataResult = mysqli_stmt_get_result($firstdataStmt);

    // Check for errors in the first query execution
    if (!$firstdataResult) {
        die('Error in first query: ' . mysqli_error($conn));
    }

    // Check the number of rows for the first query
    if (mysqli_num_rows($firstdataResult) == 1) {
        $data1 = mysqli_fetch_array($firstdataResult);
    ...
    mysqli_stmt_close($firstdataStmt);

    // Second Query
    $secondQuery = "
        SELECT *
        FROM first_trimester ft
        LEFT JOIN second_trimester st ON ft.patients_id = st.patients_id
        LEFT JOIN third_trimester tt ON st.patients_id = tt.patients_id
        WHERE ft.patients_id = ? AND
        (ft.records_count = ? OR st.records_count = ? OR tt.records_count = ?) LIMIT 1";

    $seconddataStmt = mysqli_prepare($conn, $secondQuery);
    

    mysqli_stmt_bind_param($seconddataStmt, "iiii", $patient_id, $currentCount, $currentCount, $currentCount);
    mysqli_stmt_execute($seconddataStmt);

    $seconddataResult = mysqli_stmt_get_result($seconddataStmt);

    // Check for errors in the second query execution
    if (!$seconddataResult) {
        die('Error in second query: ' . mysqli_error($conn));
    }

    // Check the number of rows for the second query
    if (mysqli_num_rows($seconddataResult) == 1) {
        $data2 = mysqli_fetch_array($seconddataResult);

        $res['data2'] = [
            'status' => 201,
            'message' => 'Trimester fetched',
        ];
    } else {
        $res['data2'] = [
            'status' => 405,
    ...
    mysqli_stmt_close($seconddataStmt);
...
if (isset($_POST['update_age'])) {
    ...
    // Use prepared statement to fetch current birthdate and age
    $select_sql = "SELECT birthdate, age FROM patients WHERE id = ?";
    ...
    $select_stmt->execute();
    $select_stmt->bind_result($birthdate, $existing_age);
    $select_stmt->fetch();
    $select_stmt->close();

    // Calculate new age based on fetched birthdate
    $current_date = new DateTime();
    $birthdate_date = new DateTime($birthdate);
    $new_age = $current_date->diff($birthdate_date)->y;

    // Check if the new age is different from the existing age in the database
    if ($new_age != $existing_age) {
        // Use prepared statement to update age
        $update_sql = "UPDATE patients SET age = ? WHERE id = ?";
        ...
        // Bind parameters
        $update_stmt->bind_param("ii", $new_age, $patient_id);
        ...
        if ($update_stmt->execute()) {
            ...
            error_log("Execution failed: " . $update_stmt->error);
            
            $res = [
                'status' => 500,
                'message' => 'Patient age not updated successfully. Please try again later.'
            ];
            echo json_encode($res);
        }
        ...
        $res = [
            'status' => 300,
            'message' => 'Patient age is already up to date',
            'age' => $new_age,
        ];
        echo json_encode($res);
    }
...
if (isset($_POST['delete_attachment'])) {
    $attachments_id = mysqli_real_escape_string($conn, $_POST['attachments_id']);

    $query = "SELECT filename FROM patients_attachments WHERE attachments_id ='$attachments_id'";
    ...
    if ($result) {
        ...
        $query = "DELETE FROM patients_attachments WHERE attachments_id ='$attachments_id'";
        $query_run = mysqli_query($conn, $query);
...
if (isset($_POST['add_patient_note'])) {
    $patients_id = $_POST['patients_id'];
    $note = $_POST['note'];
    $record = $_POST['record'];

    // Create a prepared statement for the stored procedure
    $sql = "CALL patient_note(?, ?, ?)";
    $stmt = $conn->prepare($sql);
    ...
    // Bind the parameters
    $stmt->bind_param("isi", $patients_id, $note, $record);
...
if (isset($_GET['patient_note_id'])) {
    $patients_id = $_GET['patient_note_id'];
    $record = $_GET['record'];

    // Use prepared statements to prevent SQL injection
    $query = "SELECT note FROM prenatal_records WHERE patients_id = ? AND records_count = ?";
    $stmt = mysqli_prepare($conn, $query);

    // Assuming 'i' is the correct type for patients_id and records_count; adjust if needed
    mysqli_stmt_bind_param($stmt, "ii", $patients_id, $record);
    mysqli_stmt_execute($stmt);
