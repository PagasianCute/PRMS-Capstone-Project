document.addEventListener("DOMContentLoaded", () => {
  ...
  const checkup = urlParams.get("checkup");
  ...
  selectElement.addEventListener("change", function () {
    ...
    checkTrimesterData(patientID, selectedValue);
    getTrimesterData(leftTab, rightTab, patientID, selectedValue);
    checkCheckup(leftTab, patientID, selectedValue);
    location.reload();
  });
  ...
  function proceedFunction() {
    if (trimester != null && checkup != null) {
      if (trimester == "First Trimester") {
        $("#first_trimester-tab").tab("show");
        leftTab = $("#first_trimester-tab").data("tab");
      } else if (trimester == "Second Trimester") {
        $("#second_trimester-tab").tab("show");
        leftTab = $("#second_trimester-tab").data("tab");
      } else if (trimester == "Third Trimester") {
        $("#third_trimester-tab").tab("show");
        leftTab = $("#third_trimester-tab").data("tab");
      }

      if (checkup == "First Checkup") {
        $("#first_check_up-tab").addClass("active");
        $("#first_check_up-tab").click();
        $("#second_check_up-tab, #third_check_up-tab").removeClass("active");
        rightTab = $("#first_check_up-tab").data("tab");
      } else if (checkup == "Second Checkup") {
        $("#second_check_up-tab").addClass("active");
        $("#second_check_up-tab").click();
        $("#first_check_up-tab, #third_check_up-tab").removeClass("active");
        rightTab = $("#second_check_up-tab").data("tab");
      } else if (checkup == "Third Checkup") {
        $("#third_check_up-tab").addClass("active");
        $("#third_check_up-tab").click();
        $("#first_check_up-tab, #second_check_up-tab").removeClass("active");
        rightTab = $("#third_check_up-tab").data("tab");
      }
      $("#trimesters-tab").tab("show");
    }
  }
  ...
  function withrecord() {
    ...
    checkTrimesterData(patientID, record);
    checkCheckup(leftTab, patientID, record);
  ...
  $(document).on("click", "#createNewRecords", function () {
    $.ajax({
      ...
      success: function (response) {
        ...
        if (res.status == 200) {
          // Fetch the record count from the response
          var recordCount = parseInt(res.record_count);

          // Increment the record count by 1
          var updatedRecordCount = recordCount;

          // Update the URL with the new record count value
          urlParams.set("record", updatedRecordCount);
  ...
  function checkTrimesterData(patient_id, recordsCount) {
    $.ajax({
      type: "GET",
      url: "server/prenatal_function.php",
      data: {
      ...
      success: function (response) {
        try {
          ...
          if (res.first_trimester.status == 100) {
            $("#second_trimester-tab").removeClass("disabled");
          } else if (res.first_trimester.status == 404) {
            $("#second_trimester-tab").addClass("disabled");
          }
          if (res.second_trimester.status == 200) {
            $("#third_trimester-tab").removeClass("disabled");
          } else if (res.second_trimester.status == 405) {
            $("#third_trimester-tab").addClass("disabled");
          }
        } catch (error) {
  ...
  function checkCheckup(trimester, patient_id, recordsCount) {
    $.ajax({
      type: "GET",
      url: "server/prenatal_function.php",
      data: {
        check_Checkup: true,
        new_trimester_table: trimester,
        new_patientid: patient_id,
        new_records_count: recordsCount,
      },
      success: function (response) {
        try {
          var res = jQuery.parseJSON(response);
          ...
          if (res.first_trimester.status == 100) {
            $("#second_check_up-tab").removeClass("disabled");
          } else if (res.first_trimester.status == 404) {
            $("#second_check_up-tab").addClass("disabled");
          }
          if (res.second_trimester.status == 200) {
            $("#third_check_up-tab").removeClass("disabled");
          } else if (res.second_trimester.status == 405) {
            $("#third_check_up-tab").addClass("disabled");
          }
        } catch (error) {
          console.error("Error parsing JSON response: ", error);
        }
        ...
        if (res.status == 200) {
          ...
          if (record == null) {
            if (lastRecord !== null) {
              ...
              var recordCount = parseInt(lastRecord);
          ...
          var nextPageUrl = "prenatal.php"; // Change this to the actual URL of your next page
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (res.status == 200) {
          ...
          setTimeout(function () {
            showToast("First Trimester", "Data Saved");
          }, 1000);
          updatebuttons();
        } else if (res.status == 300) {
          $("#errorMessage").removeClass("d-none");
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (res.status == 200) {
          ...
          playNotificationSound();
          setTimeout(function () {
            showToast("Second Trimester", "Data Saved");
          }, 1000);
          updatebuttons();
        } else if (res.status == 300) {
          $("#errorMessage").removeClass("d-none");
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (res.status == 200) {
          ...
          playNotificationSound();
          setTimeout(function () {
            showToast("Third Trimester", "Data Saved");
          }, 1000);
          updatebuttons();
        } else if (res.status == 300) {
          $("#errorMessage").removeClass("d-none");
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (res.status == 200) {
          ...
          playNotificationSound();
          setTimeout(function () {
            showToast("Birth Experience", "Data Saved");
          }, 1000);
        } else if (res.status == 300) {
          $("#errorMessage").removeClass("d-none");
          $("#errorMessage").text(res.message);
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (res.status == 200) {
          ...
          playNotificationSound();
          setTimeout(function () {
            showToast("Patient Information", "Data Saved");
          }, 1200);
        } else if (res.status == 300) {
          $("#errorMessage").removeClass("d-none");
          $("#errorMessage").text(res.message);
  ...
  function toastDelayed() {
    playNotificationSound();
    showToast("Record", "Record Deleted");
  }
});
