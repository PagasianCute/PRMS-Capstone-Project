document.addEventListener("DOMContentLoaded", () => {
  ...
  function getTrimesterData(trimester, checkup, patient_id, recordsCount) {
    $.ajax({
      type: "GET",
      url: "server/view_patient_prenatal_function.php",
      data: {
  ...
  function updatePatientAge() {
    ...
    formData.append("update_age", true);
    formData.append("patients_id", patientID);
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (res.status == 422) {
          ...
          playNotificationSound();
          setTimeout(function () {
            showToast("Update", "Patient's age Updated");
          }, 1000);
          reloadPatientAge(res.age);
  ...
  function reloadPatientAge(age) {
    // You may need to replace "your-paragraph-id" with the actual ID of your paragraph element
    var paragraph = $("#updated-age");

    // Assuming res.message contains the updated age
    var updatedAge = age;

    // Update the content of the paragraph
    paragraph.text(updatedAge);
  }
...
$(document).on("click", ".viewPatient", function () {
  ...
  $.ajax({
    type: "GET",
    url:
      "server/view_patient_prenatal_function.php?view_patient_id=" + rffrl_id,
    success: function (response) {
      var res = jQuery.parseJSON(response);
      if (res.status == 422) {
        alert(res.message);
  ...
  $(document).on("click", ".viewPatientRecords", function () {
    ...
    $.ajax({
      type: "GET",
      url:
        "server/view_patient_prenatal_function.php?view_patient_records_id=" +
        patients_id,
      success: function (response) {
        var res = jQuery.parseJSON(response);
        if (res.status == 404) {
          ...
          $.ajax({
            type: "GET",
            url:
              "server/view_patient_prenatal_function.php?patient_count_id=" +
              patients_id,
            success: function (response) {
              var res = jQuery.parseJSON(response);
              if (res.status == 404) {
