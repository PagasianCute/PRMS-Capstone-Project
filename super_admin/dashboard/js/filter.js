...
const facilityCheckboxes2 = document.querySelectorAll('.facility2 .btn-check');
  ...
  group.forEach(checkbox => {
    checkbox.addEventListener('change', function () {
      ...
      const filterData = {
        ...
        fclt_name2: checkedLabels.fclt_name2,
...
addEventListenerToGroup(facilityCheckboxes2, 'fclt_name2');
      ...
      if (data.status === 200) {
        ...
        data.data.forEach(function (row, index) {
          $('#referralsTable tbody').append(`
            <tr>
              <th scope="row">${row.rfrrl_id}</th>
              <td>${row.fclt_name}</td>
              <td>${row.referred_hospital_name}</td>
              <td class="action-column" id="${row.status}-column"><p>${row.status}</p></td>
              <td>${row.date} • ${row.time}</td>
              <td class="action-column">
                <button id="icon-btn" type="button" value="${row.rfrrl_id}" class="viewMyRecord">
