...
if (isset($_POST['create_referral'])) {
    ...
    if ($query_run) {
        ...
        $select_query = "SELECT * FROM staff WHERE fclt_id = '$referred_hospital' AND status = 'Active'";
        $result = $conn->query($select_query);

        if ($result) {
            $selected_data = $result->fetch_assoc();
            $contact_num = $selected_data['contact_num'];
            $message = "New referral. Please check the portal";
            sendSMS($contact_num, $message);

            $response = [
                'status' => 200,
                'message' => 'Referral data inserted successfully and sms success',
            ];
        } else {
            $response = [
                'status' => 500,
                'message' => 'Error executing SELECT statement: ' . $conn->error,
            ];
        }
