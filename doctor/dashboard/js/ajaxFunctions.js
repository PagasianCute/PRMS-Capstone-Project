document.addEventListener("DOMContentLoaded", function () {
  const searchBar = document.querySelector("#patientName #search-patient");
        ...
        if (res.status == 422) {
          ...
          for (var i = 0; i < querytransactions_data.length; i++) {
            ...
            if (status) {
              ...
              var pElement = $("<p></p>");
              pElement.text(status + " by " + fclt_name + " at " + time);
              referralTransactionsDiv.append(pElement);
            }
          }
        }
      },
  ...
  searchBar.addEventListener("input", function () {
    ...
    fetch("server/search-patient.php", {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
      },
      body: "patient_name=" + encodeURIComponent(searchterm),
    })
      ...
      .then((data) => {
        ...
        if (data.length > 0) {
          data.forEach((patient) => {
            ...
            option.setAttribute("data-patient-id", patient.patientId);
            datalistOptions.appendChild(option);
          });

          // Assuming data is a valid HTML string with options
          let options = datalistOptions.querySelectorAll("option");

          // Attach an event listener for the 'input' event on the text input
          searchBar.addEventListener("input", function () {
            // Find the closest matching option based on the input value
            ...
            if (selectedOption) {
              let patientId = selectedOption.getAttribute("data-patient-id");

              // Update the hidden input with the selected patient ID
              patientIdInput.value = patientId;
              referralPatientIdInput.value = patientId;
            } else {
              // If no matching option is found, clear the hidden input
              patientIdInput.value = "";
              referralPatientIdInput.value = "";
            }
          });
        }
      })
